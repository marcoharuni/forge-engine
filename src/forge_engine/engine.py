...
class GenerationCore:
    ...
    def __init__(
        ...
        self._model = model
        self._cache_pool = PagedKVBlockPool() if cache_pool is None else cache_pool
    ...
    def decode(
        ...
        direct_paged = bool(
            ...
            and (attention_mask is None or bool(attention_mask.bool().all().item()))
        )
        output = self._model.forward(
            input_ids=input_ids,
            attention_mask=None if direct_paged else attention_mask,
            past_key_values=(
                state.cache if direct_paged else state.cache.as_model_input()
            ),
            use_cache=True,
        )
        if direct_paged:
    ...
    for layer_index, layer in enumerate(cache):
        ...
        for name, tensor in (("key", key), ("value", value)):
            if getattr(tensor, "ndim", None) != 4:
                raise ValueError(f"cache layer {layer_index} {name} must be rank four")
            if tensor.shape[0] != batch_size or tensor.shape[-2] != sequence_length:
                raise ValueError(
            ...
            if getattr(getattr(tensor, "device", None), "type", None) != "cuda":
                raise ValueError(f"cache layer {layer_index} {name} must be on CUDA")
        if key.shape != value.shape or key.dtype != value.dtype:
            raise ValueError(f"cache layer {layer_index} key/value metadata must match")
...
def _first_stop(text: str, stop_strings: Sequence[str]) -> int | None:
    ...
    positions = [
        position for stop in stop_strings if (position := text.find(stop)) >= 0
    ]
...
def _split_cache_batch(
    ...
    requests: list[list[tuple[object, object]]] = [[] for _ in range(batch_size)]
    for layer_index, layer in enumerate(past_key_values):
        if not isinstance(layer, (tuple, list)) or len(layer) != 2:
            raise ValueError(f"cache layer {layer_index} must contain key and value")
        key, value = layer
